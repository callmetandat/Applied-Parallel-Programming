        ...
        if isOutputPixel:
            for r in range(KERNEL_SIZE):
                for c in range(KERNEL_SIZE):
                    outPixel += weight[out_channel_idx, in_channel_idx, r, c] * sharedImg[shared_row + r, shared_col + c]
...
def Convolution2D_GPU(img, weight, bias):
    ...
    out_img = np.zeros((BATCH_SIZE, OUT_CHANNEL, IMG_HEIGHT, IMG_WIDTH), dtype=np.float32)
